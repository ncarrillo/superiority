...
use superiority_ui::products::sc2::{
    MembershipEvent, MembershipKind, PresenceKind, RosterChannelKind, RosterPresentation,
    RosterRelationship, RosterUser, RosterUserTone, Sc2Assets, TranscriptLine,
    components::{roster, workspace},
};
use wasm_bindgen::JsValue;

use super::super::api::{ChannelSummary, RosterMember, Sender, StreamItem};
...
pub(super) fn presented_roster_members(
    ...
    let mut members = members
        ...
        .map(|mut member| {
            ...
            let wears_local_clan = member.clan_tag.as_ref().is_some_and(|clan_tag| {
                local_clan
                    .as_ref()
                    .is_some_and(|local_clan| clan_tag.to_lowercase() == *local_clan)
            });
...
pub(super) fn stream_line(item: &StreamItem, assets: &Sc2Assets) -> TranscriptLine {
    let StreamItem::Message(message) = item;
    let time = stamp_label(message.ts);
    match message.kind.as_str() {
        "member_joined" | "member_left" => {
            let kind = if message.kind == "member_left" {
                MembershipKind::Left
            } else {
                MembershipKind::Joined
            };
            match message.sender.as_ref() {
                Some(sender) => TranscriptLine::Membership(MembershipEvent {
                    time,
                    kind,
                    members: vec![sender_user(sender, assets)],
                    expanded: false,
                }),
                None => TranscriptLine::Notice {
                    time,
                    text: format!("Someone {}", kind.verb()),
                },
            }
        }
        "broadcast" | "information" => TranscriptLine::Notice {
            time,
            text: message.body.clone(),
        },
        _ => match message.sender.as_ref() {
            Some(sender) => TranscriptLine::Message {
                time,
                sender: sender_user(sender, assets),
                text: message.body.clone(),
            },
            // a senderless talk line the transcript cannot attribute
            None => TranscriptLine::Notice {
                time,
                text: message.body.clone(),
            },
...
fn sender_user(sender: &Sender, assets: &Sc2Assets) -> RosterUser {
    RosterUser {
        handle: sender.handle,
        name: sender.display_name(),
        presence_id: None,
        presence: PresenceKind::Unknown,
        presence_label: PresenceKind::Unknown.label().to_owned(),
        presence_icon: assets.presence_icon(PresenceKind::Unknown),
        portrait: None,
        clan_tag: sender.clan_tag.clone(),
        clan_is_local: false,
