...
impl RosterComponent {
    ...
    fn scroll_layer(
        ...
        let layer = if interactive {
            layer = layer
                ...
                .on_scroll_wheel(cx.listener(|this, _, _, cx| {
                    if this.session.roster.roster.animation.take().is_some() {
                        cx.notify();
                    }
                }))
                .on_hover(cx.listener(|this, hovered, _, cx| {
                    this.session.roster.set_hovered(*hovered);
                    cx.notify();
                }));
            if animating {
                            ...
                            let entries = this
                                ...
                                .map(|channel| {
                                    presented_roster_range(
                                        &this.session.channels.tabs,
                                        &this.session.social.friends,
                                        channel,
                                        &channel.roster_filter,
                                        range,
                                    )
        ...
        if interactive {
            let scroll = self.roster.scroll.0.borrow().base_handle.clone();
            layer.vertical_scrollbar_in(
                &scroll,
                ui_shared_modal::ModalVariant::Sc2.scrollbar(),
                window,
                cx,
            )
        } else {
    ...
    fn header(
        ...
        if channel.is_none_or(|channel| channel.roster_filter.is_empty()) {
            ...
            header.on_clear(cx.listener(|this, _, _, cx| {
                this.session.roster.roster_input.clear();
