...
impl ChannelComponent {
    pub(in crate::app::client) fn tab_effect_opacity(&self, effect: usize, now: Instant) -> f32 {
    ...
    pub(in crate::app::client) fn view(
        ...
        let tabs = self
            ...
            .iter()
            .enumerate()
            // the party is the dock above the input, not a tab: a tab would
            // duplicate it and take the party's talk out of the transcript
            .filter(|(_, tab)| tab.channel.as_ref() != Some(&ChatChannel::Party))
            .map(|(index, tab)| {
                let id = tab.id;
        ...
        let account = div()
            ...
            .child(
                account_portrait
                    ...
                    .absolute()
                    .top(px(ui_account_pattern::PORTRAIT_INSET))
                    .left(px(ui_account_pattern::PORTRAIT_INSET))
                    .size(px(ui_account_pattern::PORTRAIT_SIZE))
        ...
        let account = account
            .left_0()
            .rounded_tl(px(ui_account_pattern::CORNER_RADIUS));
        #[cfg(target_os = "macos")]
        let account = account
            .right_0()
            .rounded_tr(px(ui_account_pattern::CORNER_RADIUS));
