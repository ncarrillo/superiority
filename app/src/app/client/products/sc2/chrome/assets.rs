...
impl PortraitRegistry {
    ...
    #[cfg(test)]
    pub(in crate::app::client) fn empty() -> Self {
        Self {
            atlases: Vec::new(),
            cells: BTreeMap::new(),
        }
...
pub(in crate::app) fn load_fonts(root: &std::path::Path, cx: &mut App) {
    let fonts = [
        ...
        "fonts/bl.ttf",
        // Friz Quadrata, Reforged's UI face — the WC3 card and realm ask for it
        "fonts/frizqt.ttf",
    ]
    .into_iter()
...
mod tests {
    ...
    #[test]
    fn a_product_with_no_avatar_model_asks_for_nothing_and_gets_nothing() {
        // Remastered is not expected to have StarCraft II's portrait set, and
        // its users could not index these cells regardless — the keys are a
        // StarCraft II wire type. An empty registry has to degrade to the
        // placeholder frame rather than panic.
        let mut registry = PortraitRegistry::empty();
        assert!(
            registry
                .image(ImageTableEntry {
                    table_id: 0,
                    offset: 0,
                })
                .is_none()
        );
        // and an offset past the end of a row is refused the same way
        assert!(
            registry
                .image(ImageTableEntry {
                    table_id: 0,
