...
impl ChannelState {
    ...
    /// a joined party, for previewing the dock. a real one only ever arrives
    /// by invitation, so there is otherwise no way to see the strip without a
    /// second account.
    pub(in crate::app::client) fn fixture_party(id: u64) -> Self {
        Self {
            channel: Some(ChatChannel::Party),
            channel_index: Some(1),
            title: "Party".to_owned(),
            users: (0..3).map(UiUser::fixture).collect(),
            ..Self::fixture_joined(id, "Party".to_owned())
        }
    }
...
pub(in crate::app::client) fn join_rejection_notice(channel: &str, reason: Option<u16>) -> String {
    ...
    if let Some(message) = superiority_core::native::errors::localized_message(code) {
        return format!(
    ...
    match code {
        ...
        _ => match superiority_core::native::errors::message(code) {
