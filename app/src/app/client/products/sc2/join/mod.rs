...
/// party invitations live in the transcript rather than the stack, so what is
/// kept here is only the record needed to answer one.
pub(in crate::app::client) const INVITATION_RECORDS: usize = 32;
...
pub(in crate::app::client) struct JoinComponent {
    pub(in crate::app::client) awaiting_joins: Vec<(ChatChannel, Instant)>,
    pub(in crate::app::client) group_search_due: Option<(Instant, String)>,
    pub(in crate::app::client) public_channels: BTreeMap<u16, String>,
    pub(in crate::app::client) groups: BTreeMap<u32, UiGroupSummary>,
    pub(in crate::app::client) remembered_group_names: BTreeMap<u32, String>,
    pub(in crate::app::client) member_groups: BTreeSet<u32>,
    pub(in crate::app::client) group_search: Vec<u32>,
    pub(in crate::app::client) invitations: Vec<UiInvitation>,
    pub(in crate::app::client) next_invitation_id: u64,
    ...
    pub(in crate::app::client) channel_conferences: BTreeMap<u16, Vec<u32>>,
    /// live head count per conference, from the most recent counts page.
    pub(in crate::app::client) conference_members: BTreeMap<u32, u16>,
    /// set once a complete directory has arrived. until then nothing is pruned,
    /// because an empty set would mean an empty list.
    pub(in crate::app::client) directory_complete: bool,
}

impl JoinComponent {
    pub(in crate::app::client) fn schedule_group_search(&mut self, query: &str) {
    ...
    pub(in crate::app::client) fn flush_group_search_if_due(
        &mut self,
    ...
    pub(in crate::app::client) fn target_for_query(&self, query: &str) -> ChatChannel {
