...
impl SuperiorityView {
    ...
    pub(in crate::app::client) fn set_connection_progress_stage(&mut self, stage: ConnectionStage) {
        ...
        self.session.connection.floor = floor;
        self.session.connection.ceiling = ceiling;
        if index.is_none() || floor >= 1.0 {
            self.session.connection.fill = floor;
        } else {
            // never behind the stage it is reporting. The creep that would
            // carry it there only runs while the connection dialog is up, and
            // a card shows this bar with no dialog at all — so its fill stayed
            // at zero through every step and then jumped at the end.
            self.session.connection.fill = self.session.connection.fill.max(floor);
        }
        self.session.connection.progress_updated = Instant::now();
    }

    pub(in crate::app::client) fn show_disconnect_warning(&mut self, detail: String) {
        self.overlays.active = None;
        self.overlays.closing = false;
        self.session.connection.dialog_visible = false;
        self.session.connection.dialog_closing = false;
        self.session.connection.close_due = None;
        self.session.connection.hide_due = None;
