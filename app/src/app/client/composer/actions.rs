...
impl SuperiorityView {
        ...
        if let Some(word) = command_word(&body) {
            ...
            match (kind, query) {
                (Some(CommandKind::Join), Some(query)) => {
                    let target = self.join.target_for_query(&query);
    ...
    pub(in crate::app::client) fn command_results(&self) -> Option<CommandResults> {
        self.results_for(&self.composer.active_command()?)
    }
    ...
    fn results_for(&self, command: &ChatCommand) -> Option<CommandResults> {
        ...
        let rows = match command.kind {
            CommandKind::Join => {
                let (rows, offer) =
                    self.join
                        .command_matches(&self.channels.tabs, &command.query, COMMAND_RESULTS);
                create = offer;
            ...
            .collect(),
            CommandKind::Mention => {
                mention_candidates(self.channels.active(), &command.query, COMMAND_RESULTS)
                    .into_iter()
                    .map(CommandRow::Person)
                    .collect()
            }
    ...
    pub(in crate::app::client) fn begin_whisper(
        ...
        self.social.present_conversation(peer, sliding, window, cx);
        cx.notify();
    }
    ...
    fn send_opening_whisper(&mut self, message: String) {
        ...
        self.social.conversation_input.set_content(message);
    ...
    pub(in crate::app::client) fn insert_mention(&mut self, name: &str, span: &Range<usize>) {
        let line = &self.composer.command_line;
        ...
        self.composer.composer.set_content(line);
        self.composer.composer.set_cursor(caret);
    }
    ...
    pub(in crate::app::client) fn open_join_command(
        ...
        let was_open = self.composer.active_command().is_some();
        self.roster.roster.focused = false;
        self.join.group_search_due = None;
        self.join.group_search.clear();
        self.composer.composer.set_content(command_prefix());
    ...
    pub(in crate::app::client) fn clear_composer_command(&mut self, cx: &mut Context<Self>) {
        ...
        self.composer.composer.clear();
        self.composer.command_line.clear();
        self.composer.command_selected = 0;
        self.composer.command_dismissed = false;
    ...
    pub(in crate::app::client) fn begin_command_close(
        ...
        self.composer.command_closing = Some(results);
        self.composer.command_close_epoch = self.composer.command_close_epoch.wrapping_add(1);
        let epoch = self.composer.command_close_epoch;
        ...
        cx.spawn(async move |entity, cx| {
            ...
            entity
                .update(cx, |this, cx| {
                    if this.composer.command_close_epoch == epoch {
                        this.composer.command_closing = None;
                        cx.notify();
                    }
                })
                .ok();
    ...
    pub(in crate::app::client) fn cancel_command_close(&mut self) {
        if self.composer.command_closing.take().is_some() {
            self.composer.command_close_epoch = self.composer.command_close_epoch.wrapping_add(1);
        }
    }
}
