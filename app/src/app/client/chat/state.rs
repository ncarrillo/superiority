...
impl SuperiorityView {
    ...
    pub(in crate::app::client) fn append_membership(
        ...
        if member.portrait.is_none() || member.clan_tag.is_none() {
            self.channels.tabs[index].identities_pending = true;
        }
        if self.channels.tabs[index].users.len() > HIGH_TRAFFIC_ONLINE {
            self.append_digest(index, kind, member);
            return;
        }
        let Some(tab) = self.channels.tabs.get_mut(index) else {
            return;
        };
        let now = Instant::now();
    ...
    pub(in crate::app::client) fn toggle_digest_expansion(
        ...
        let now = Instant::now();
        if let Some(expanded) = &mut self.chat.expanded_digest
        ...
        self.chat.expanded_digest = Some(ExpandedDigest {
            tab_id,
            line_index,
            started: now,
            closing: false,
