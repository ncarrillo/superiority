...
pub const TAB_BAR_HEIGHT: f32 = crate::patterns::account::HEADER_HEIGHT;
...
/// a focus glow in a colour other than the accent — the composer wears one
/// while it is addressed somewhere other than the channel, so where the next
/// line is going is unmissable before you press enter.
#[must_use]
pub fn scope_glow(color: u32) -> Vec<BoxShadow> {
    vec![BoxShadow::new(px(0.0), px(0.0), rgba(color).into()).blur_radius(px(12.0))]
}
