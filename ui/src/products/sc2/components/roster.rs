use gpui::{
    ...
    ObjectFit, Render, RenderOnce, SharedString, Stateful, StyledImage as _, Window, div, img,
    prelude::*, px, rgb, rgba,
};

use crate::products::sc2::{
    Portrait, RosterSegment, RosterUser, RosterUserTone, Sc2Assets,
    theme::{
...
pub use crate::patterns::roster::{
    PlacedRows, RowMotion, TimedTransition, Transition, filter_matches, filtered_count,
    filtered_range, filtered_refs, placed_rows,
};
...
#[must_use]
pub fn animated_rows<S, T: Clone, C: crate::foundation::animation::AnimationClock>(
    items: Vec<T>,
    animation: Option<&TimedTransition<S, T, C>>,
    now: C,
    handle: impl Fn(&T) -> u32,
    row_gap: f32,
    render: impl FnMut(&T, RowMotion) -> AnyElement,
) -> Vec<AnyElement> {
    crate::patterns::roster::animated_rows(
        items,
        animation,
        now,
        handle,
...
#[must_use]
pub fn animated_rows_with_height<S, T: Clone, C: crate::foundation::animation::AnimationClock>(
    items: Vec<T>,
    animation: Option<&TimedTransition<S, T, C>>,
    now: C,
    handle: impl Fn(&T) -> u32,
    ...
    crate::patterns::roster::animated_rows(
        items, animation, now, handle, row_height, row_gap, render,
    )
}

#[must_use]
pub fn virtual_row_slot(row: impl IntoElement, row_gap: f32) -> Div {
    crate::patterns::roster::virtual_row_slot(row, ROSTER_ROW_HEIGHT, row_gap)
}
...
fn portrait(user: &RosterUser, assets: &Sc2Assets) -> Div {
    framed_portrait(
...
#[must_use]
pub fn inline_portrait(user: &RosterUser, assets: &Sc2Assets, size: f32) -> AnyElement {
    div()
        .relative()
        .size(px(size))
        .flex_shrink_0()
...
impl RosterTooltip {
    #[must_use]
    pub fn new(user: RosterUser, channel: String, assets: Sc2Assets) -> Self {
        Self {
            user,
            channel,
            assets,
...
pub struct RosterHeader {
    ...
    tooltip: Option<(ChannelTooltipModel, Sc2Assets)>,
}
...
impl RosterHeader {
    ...
    #[must_use]
    pub fn channel_tooltip(mut self, model: ChannelTooltipModel, assets: Sc2Assets) -> Self {
...
impl ChannelTooltip {
    #[must_use]
    pub fn new(model: ChannelTooltipModel, assets: Sc2Assets) -> Self {
        Self { model, assets }
    }
}
...
#[must_use]
fn row_body(user: &RosterUser, assets: &Sc2Assets) -> Div {
    div()
        .relative()
        .size_full()
        .flex()
...
#[must_use]
pub fn person_row(user: &RosterUser, assets: &Sc2Assets) -> Div {
    row_body(user, assets)
}

#[must_use]
pub fn static_row(user: &RosterUser, assets: &Sc2Assets, selected: bool) -> Div {
    div()
        .relative()
        .h(px(ROSTER_ROW_HEIGHT))
        .w_full()
