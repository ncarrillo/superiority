...
use sc2_core::metadata::{IntegerRange, ProtocolMetadata, TypeKind, TypeShape, read_metadata};
...
fn main() {
    ...
    let mut root = String::from(
        "// @generated by bsn-schema-generator (cargo run -p bsn-schema-generator) — do not edit.\n\
         //! Typed BSN messages, one module per protocol domain.\n\n",
    );
...
fn emit_wire_schema(meta: &ProtocolMetadata, ids: &BTreeSet<u32>) -> String {
    let mut out = String::from(
        "// @generated by bsn-schema-generator (cargo run -p bsn-schema-generator) — do not edit.\n\
         //! Compiled BSN schema used by the runtime bit codec.\n\n\
         #![allow(clippy::unreadable_literal)]\n\n\
         use crate::metadata::{IntegerRange, StaticProtocolSchema, StaticTypeShape, TypeKind};\n\n",
    );
    writeln!(
        out,
        "#[rustfmt::skip]\npub static SCHEMA: StaticProtocolSchema = StaticProtocolSchema::new({}, &[",
...
fn emit_struct(out: &mut String, meta: &ProtocolMetadata, id: u32, shape: &TypeShape) {
...
fn emit_enum(out: &mut String, meta: &ProtocolMetadata, id: u32, shape: &TypeShape) {
    let name = type_name(meta, id);
    let mut used = HashSet::new();
...
fn emit_choice(out: &mut String, meta: &ProtocolMetadata, id: u32, shape: &TypeShape) {
    let name = type_name(meta, id);
    let mut used = HashSet::new();
...
fn field_type(meta: &ProtocolMetadata, id: u32) -> String {
    let shape = meta.shape(id).unwrap();
    match shape.kind {
...
fn module_of(meta: &ProtocolMetadata, id: u32) -> String {
...
fn raw_name(meta: &ProtocolMetadata, id: u32) -> String {
...
fn type_name(meta: &ProtocolMetadata, id: u32) -> String {
    let raw = raw_name(meta, id);
