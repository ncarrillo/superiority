...
use crate::{
    ...
    native::model::{
        ...
        ConferenceMemberCount, ConferenceMemberCounts, ConnectionMessageFrame, FriendEntry,
        FriendIdentity, FriendToon, FriendToonPage, FriendUpdate, FriendsPage, MemberClanTag,
        MembershipChange, MembershipKind, PartyMemberStatus, Payload, PresenceField,
        PresenceFieldFlags, PresenceFields, PresenceUpdate, ProfileAddress, ProfileReadResponse,
        ProfileReadResult, SocialOperation, StartupSummary, ToonDisplay, ToonFullName, ToonHandle,
        ToonList, ToonNameResolved, ToonSelected,
    },
...
pub(crate) fn chat_whisper_with_provenance(
    ...
    let read_marker =
        |reader: &mut BitReader<'_>, provenance: &mut Vec<DecodedField>| -> Result<BsnValue> {
            traced_reflected_field(
                protocol,
                type_id,
                marker,
    ...
    let read_peer =
        |reader: &mut BitReader<'_>, provenance: &mut Vec<DecodedField>| -> Result<ToonFullName> {
            let peer_start = reader.position();
            let peer = trace_chat_display_name(reader, "value.peer", 2, provenance)?;
            provenance.push(decoded_field(
                "value.peer",
                "ToonFullName",
                "4 fields",
    ...
    let read_body = |reader: &mut BitReader<'_>,
                     provenance: &mut Vec<DecodedField>|
    ...
    let (peer, body) = if echo {
        reader.read(18)?;
        let body = read_body(reader, &mut provenance)?;
        read_marker(reader, &mut provenance)?;
        let peer = read_peer(reader, &mut provenance)?;
        (peer, body)
    } else {
        read_marker(reader, &mut provenance)?;
        let peer = read_peer(reader, &mut provenance)?;
...
pub(crate) fn toon_blocks(protocol: &Protocol, reader: &mut BitReader<'_>) -> Result<Payload> {
    Ok(toon_blocks_with_provenance(protocol, reader)?.payload)
}

/// `Friends/33 ToonBlockNotify`: the account's blocked-toon list — `m_blocks`,
/// then the optional `m_isEndOfList`. Each `ToonBlockContainer` is its
/// `Toon::FullName` followed by the one-bit `Update` choice (0 Add, 1
/// Remove): metadata order with no obfuscation filler, read straight off the
/// client's own readers in the 97563 image — `ToonBlockNotify`'s decoder
/// (`0x100f2b450`) calls the FullName reader then the choice reader for every
/// element, and the choice reader (`0x100f2ae30`) is a single range-checked
/// bit. An account that has blocked somebody used to fail its whole
/// connection here; the snapshot is consumed in full now. The names are not
/// surfaced yet — the blocked list is moving to the social pane — so the
/// payload stays the startup summary, with the count it actually carried.
pub(crate) fn toon_blocks_with_provenance(
    protocol: &Protocol,
    reader: &mut BitReader<'_>,
) -> Result<DecodedPayload> {
    let start_bit = reader.position();
    let count = read_spanned_usize(reader, 7)?;
    if count.value > 64 {
        return Err(native_error("toon block snapshot is too large"));
    }
    let container_type = protocol
        .codec()
        .schema()
        .unique_type_id("Battlenet::Friends::ToonBlockContainer")?;
    let toon_name_type =
        protocol.peel_alias(protocol.member_type(container_type, "m_toonName")?)?;
    let mut provenance = vec![spanned_field("value.count", "array length", &count, 1)];
    for index in 0..count.value {
        let path = format!("value.entries[{index}]");
        let item_start = reader.position();
        let toon = protocol.codec().decode_reflected_traced_from(
            reader,
            toon_name_type,
            &format!("{path}.toon"),
            3,
        )?;
        // the value is decoded to prove the bits are a toon name, not to keep
        toon_full_name_from_value(&toon.value)?;
        provenance.extend(toon.fields);
        let update = read_spanned_u64(reader, 1)?;
        provenance.extend([
            decoded_field(
                path.clone(),
                "ToonBlockContainer",
            ...
            decoded_field(
                format!("{path}.update"),
                "choice",
                if update.value == 0 { "Add" } else { "Remove" }.to_owned(),
                update.start_bit,
                update.end_bit,
                3,
            ),
...
fn collect_club_counts(value: &BsnValue, counts: &mut BTreeMap<u32, ClubCounts>) {
    if let BsnValue::Struct(fields) = value {
        ...
        if let (Some(club_id), Some(member_count)) = (
            struct_u32(fields, "m_id"),
            struct_u32(fields, "m_memberCount"),
        ) {
            let entry = counts.entry(club_id).or_default();
...
mod tests {
    ...
    #[test]
    fn toon_blocks_consume_every_blocked_toon_and_the_end_marker() {
        ...
        let container_type = protocol
            ...
            .unique_type_id("Battlenet::Friends::ToonBlockContainer")
            .unwrap();
        let full_name_type = protocol
            .peel_alias(protocol.member_type(container_type, "m_toonName").unwrap())
            .unwrap();
        let shape = protocol.codec().schema().shape(full_name_type).unwrap();
        let full_name = |name: &str| {
            let values = [
                BsnValue::Integer(1),
                BsnValue::FourCc(0x5332_0000),
                BsnValue::Integer(1),
                BsnValue::String(name.to_owned()),
            ];
            BsnValue::Struct(BsnStruct::new(
                full_name_type,
                shape
                    .index_values
                    .iter()
                    ...
                    .zip(values)
                    .map(|((index, member), value)| {
                        BsnField::named(*index, member.as_deref().unwrap(), value)
                    })
                    .collect(),
            ))
        };
        let mut writer = BitWriter::new();
        // two blocked toons: one still blocked, one being unblocked
        writer.write(2, 7).unwrap();
        for (name, removed) in [("Nova", 0), ("Tosh", 1)] {
            protocol
                .codec()
                .encode_reflected_into(&mut writer, full_name_type, &full_name(name))
                .unwrap();
            writer.write(removed, 1).unwrap();
        }
        // end of list: present, true
        ...
        let decoded = toon_blocks_with_provenance(&protocol, &mut reader).unwrap();
        let Payload::StartupSummary(summary) = decoded.payload else {
            panic!("expected the toon-block summary");
        };
        assert_eq!(summary.kind, "toon-blocks");
        ...
        let update = |index: usize| {
            decoded
                .provenance
                .iter()
                .find(|field| field.path == format!("value.entries[{index}].update"))
                .map(|field| field.value.clone())
        };
        assert_eq!(update(0).as_deref(), Some("Add"));
        assert_eq!(update(1).as_deref(), Some("Remove"));
        assert!(decoded.provenance.iter().any(|field| {
            field.path == "value.entries[1].toon.m_name" && field.value.contains("Tosh")
        }));
    }
    ...
    fn retail_conference_descriptions_name_every_public_channel() {
        ...
        assert_eq!(
            general,
            (1..=12).collect::<std::collections::BTreeSet<u16>>()
        );
