...
use super::{error_catalog, portrait_catalog, public_channels};
...
const ERROR_CATALOG_TOKEN: u32 = 1;
const PUBLIC_CHANNEL_CATALOG_TOKEN: u32 = 2;
...
use crate::{
    ...
    bgs::fourcc,
    native::{
        AccountBlockEntry, AdvertHandle, FriendEntry, FriendIdentity, FriendsPage, ImageTableEntry,
        Payload, PresenceState, ProfileAddress, ProfileReadResponse, ProfileReadResult, Protocol,
        Record, Session, SocialOperation, ToonFullName, WhisperTarget,
        model::{
            ChatJoin, ConferenceDescription, ConferenceDescriptions, ConferenceMemberCount,
            ConferenceMemberCounts, MembershipKind, PartyMemberStatus, ToonHandle, ToonList,
        },
        presence::PresenceDirectory,
        protocol::S2_MASTER_LOBBY_PREVIEW_COMMAND,
    },
};
...
impl ChannelState {
    fn begin_join(&mut self, channel: ChatChannel, local_member_handle: u32) {
        self.channel = Some(channel);
        self.local_member_handle = Some(local_member_handle);
        self.roster.clear();
        self.next_roster_order = 0;
        self.initial_roster_complete = false;
    }
}
...
impl LiveChat {
    ...
    pub fn establish(
        ...
        request_bootstrap_catalogs(&mut session, &protocol)?;
        ...
        let mut initial_join_token = None;
        ...
        loop {
            ...
            match &record.value {
                ...
                Payload::CacheStreamItems(response)
                    if response.token == PUBLIC_CHANNEL_CATALOG_TOKEN =>
                {
                ...
                Payload::ChatJoin(result) => {
                    ...
                    let Some(expected_token) = initial_join_token else {
                        events.push(ChatEvent::Activity {
                            route: record_route(&record),
                        });
                        continue;
                    };
                    if !join_result_matches_request(result, &initial_channel, expected_token) {
                        events.push(ChatEvent::Activity {
                            route: record_route(&record),
                        });
                        continue;
                    ...
                    channel.begin_join(joined.clone(), member_handle);
                ...
                Payload::CommandResponse(response) => {
                    trace_battle_net_error(
                        format_args!("command {} response", response.command_id),
                        response.result,
                    );
                    events.push(ChatEvent::Activity {
                        route: record_route(&record),
    ...
    pub fn send_whisper(
        ...
        match target {
            ...
            WhisperTarget::WarcraftAccount(_) => Err(protocol_error(
                "WC3 whisper target cannot be used on StarCraft II",
            )),
    ...
    pub fn receive(&mut self) -> Result<ChatEvent> {
        ...
        let event = match &record.value {
            ...
            Payload::ChatMessage(message) => {
                self.trace_lobby_links(&message.body);
            ...
            Payload::ChatJoin(result) => {
                    ...
                    && (result.channel_type == Some(PARTY_CHANNEL_TYPE) || rejoins_party)
                {
                    ...
                    if let Some(joined) = joined {
                        channel.begin_join(joined.clone(), local_member_handle);
                        Ok(ChatEvent::Joined {
                            channel_index,
                            channel: joined,
                            local_member_handle,
    ...
    fn probe_when_due(&mut self) -> Result<()> {
        ...
        eprintln!(
            "superiority: probe -> Chat/{command} (empty payload, {} bytes)",
            packet.len()
        );
        self.session.records_mut().send(&packet)
    }
    ...
    fn advance_pending_whispers(&mut self, payload: &Payload) -> Result<()> {
        ...
        if let Payload::ToonNameResolved(resolved) = payload {
                ...
                && let Some(handle) = resolved.handle
            {
                ...
                self.request_temporary_presence(handle)?;
            } else if resolved.result != 0 {
                let peers = self
                    .pending_whispers
                    .iter()
                    .filter(|pending| pending.toon_name.as_ref() == Some(&resolved.name))
                    .map(|pending| pending.display_name.clone())
                    .collect::<Vec<_>>();
                self.pending_whispers
                    .retain(|pending| pending.toon_name.as_ref() != Some(&resolved.name));
                let reason = crate::native::errors::description(resolved.result);
                self.pending_events.extend(peers.into_iter().map(|peer| {
    ...
    /// name.
    fn trace_lobby_links(&mut self, body: &str) {
        if std::env::var_os("SUPERIORITY_LOBBY_TRACE").is_none() {
            return;
        }
        for advert_id in lobby_link_advert_ids(body) {
            let Some(handle) = self.profiles.presence.resolve_advert_id(advert_id) else {
                eprintln!(
                    "superiority: lobbyLink {advert_id} -> no server known yet \
                     (nobody visible is in a joinable game)"
                );
                continue;
            };
            eprintln!(
                "superiority: lobbyLink {advert_id} -> label={} epoch={} advert={}",
                handle.server_label, handle.server_epoch, handle.advert_id
            );
            // asking what is *in* the lobby costs the connection, so it is not
            // something a trace does behind your back — see the probe below
            if std::env::var_os("SUPERIORITY_LOBBY_PROBE").is_none() {
                continue;
            }
            eprintln!(
                "superiority: lobby probe -> S2Master/{S2_MASTER_LOBBY_PREVIEW_COMMAND} for \
                 {advert_id}; the reply routes nowhere yet, so this session ends when it lands"
            );
            if let Err(error) = self.request_lobby_preview(&handle) {
                eprintln!("superiority: lobby probe -> request failed: {error}");
            }
        }
    }

    fn request_lobby_preview(&mut self, handle: &AdvertHandle) -> Result<()> {
        let record = self.session.records().protocol().lobby_preview_request(
            handle.server_label,
            handle.server_epoch,
            handle.advert_id,
        )?;
...
impl ProfileResolver {
    ...
    fn receive(&mut self, response: &ProfileReadResponse) -> bool {
        ...
        match &response.result {
            ...
            ProfileReadResult::Failure(code) => {
                trace_battle_net_error("profile read", *code);
                self.pending.remove(&response.request_id);
                self.avatars.insert(address, None);
                true
            }
...
fn join_result_matches_request(result: &ChatJoin, channel: &ChatChannel, token: u32) -> bool {
    result.token.map_or_else(
        || join_result_matches_channel(result, channel),
        |actual| actual == token,
    )
}
...
fn validate_public_join(requested: &ChatChannel, result: &ChatJoin) -> Result<()> {
    ...
    if let Some(actual) = result.channel_name_id
        && actual != *expected
    {
...
fn accepted_join(result: &ChatJoin) -> Result<(u8, u32)> {
    if !result.success {
        let detail = result.reason.map_or_else(
            || "without an error code".to_owned(),
            crate::native::errors::description,
        );
        return Err(Error::Server(format!(
            "Battle.net rejected the chat join: {detail}"
        )));
    }
...
impl CommandProbe {
    ...
    fn from_environment() -> Self {
        ...
        if !queue.is_empty() {
            eprintln!(
                "superiority: probe candidates {queue:?}, {}s apart",
                Self::SPACING.as_secs()
            );
        }
        Self {
            queue,
...
fn request_bootstrap_catalogs(session: &mut Session, protocol: &Protocol) -> Result<()> {
    session
        .records_mut()
        .send(&protocol.cache_get_stream_items(ERROR_CATALOG_TOKEN, "BNET", "ERRS", "enUS")?)?;
    session.records_mut().send(&protocol.cache_get_stream_items(
        PUBLIC_CHANNEL_CATALOG_TOKEN,
        "BNET",
        "CONF",
        "enUS",
    )?)
    ...
    for change in &membership.changes {
        match change.kind {
            ...
            MembershipKind::Status => {
                roster.entry(change.member_handle).or_insert_with(|| {
...
mod tests {
    ...
    #[test]
    fn a_lobby_link_is_read_for_the_advert_id_it_carries() {
        assert_eq!(
            lobby_link_advert_ids("join lobbyLink(6154076,Big Game Hunters); now"),
            vec![6_154_076]
        );
        // several in one line, which is how people post them
        assert_eq!(
            lobby_link_advert_ids("lobbyLink(9,A); or lobbyLink(6153232,B)"),
            vec![9, 6_153_232]
        );
        // and nothing to read out of an ordinary message
        assert!(lobby_link_advert_ids("no links here").is_empty());
        assert!(lobby_link_advert_ids("lobbyLink(abc,B)").is_empty());
    }
    ...
    #[test]
    fn beginning_a_rejoin_clears_the_previous_roster_bootstrap_state() {
        let mut channel = ChannelState {
            channel: Some(ChatChannel::Private("old".into())),
            local_member_handle: Some(7),
            roster: BTreeMap::from([(17, RosterMember::default())]),
            next_roster_order: 8,
            initial_roster_complete: true,
        };

        channel.begin_join(ChatChannel::Public(GENERAL_PUBLIC_CHANNEL), 91);

        assert_eq!(
            channel.channel,
            Some(ChatChannel::Public(GENERAL_PUBLIC_CHANNEL))
        );
        assert_eq!(channel.local_member_handle, Some(91));
        assert!(channel.roster.is_empty());
        assert_eq!(channel.next_roster_order, 0);
        assert!(!channel.initial_roster_complete);
    }
    ...
    #[test]
    fn public_join_accepts_an_omitted_redundant_channel_id() {
        let result = ChatJoin {
            success: true,
            channel_index: Some(2),
            member_handle: Some(91),
        ...
        assert!(validate_public_join(&ChatChannel::Public(1028), &result).is_ok());
    }

    #[test]
    fn auxiliary_join_does_not_match_a_public_request_without_its_token() {
        let result = ChatJoin {
            success: true,
            channel_index: Some(2),
            member_handle: Some(91),
        ...
        assert!(!join_result_matches_request(
            &ChatJoin {
                token: None,
                ..result.clone()
            },
